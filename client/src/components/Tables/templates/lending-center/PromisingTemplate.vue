...
<template>
  ...
  <BaseTable :title="title" :fields="fields" :rows="rows" :screenTitle="screenTitle" :onSort="onSort">
    ...
    <div v-if="screenTitle != 'Lending'">
      <b-form-row>
                <input type="checkbox" class="isCollection" :id= "data.item.screenTrackingId" :value= "data.item.screenTrackingId" v-on:change="onCheckboxInput($event, data.item.email , title , data.item.screenTrackingId , data.item.status)">
    ...
    <template #cell(promisetopayStatus)="data">
      {{ data.item.collectionPromiseStatus }}
    </template>
    <template #cell(promisedDate)="data">
      {{ data.item.collectionPromiseDate | date }}
    </template>
    <template #cell(promisedAmount)="data">
      {{ data.item.collectionPromiseAmount | currency }}
    </template>
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "promisedDate", label: "Promised Date" },
        { key: "promisedAmount", label: "Promised Amount" },
        { key: "promisetopayStatus", label: "Promise to pay Status" },
