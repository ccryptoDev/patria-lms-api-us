<template>
  <BaseTableSection :defaultParams="defaultParams" @fetch-data="fetchTable" @navigation="rows = []"
    :totalRows="totalRows" :useSearch="true" :usePagination="true">
    <LogActivityTabTemplate :rows="rows" />
  </BaseTableSection>
</template>
...
import LogActivityTabTemplate from "@/admin-dashboard/audit-log/components/LogActivityTabTemplate.vue";
  ...
  methods: {
    async fetchTable(event: TableRequestEvent) {
      ...
      try {
        const {
          data,
        } = await adminDashboardRequests.getAllLogActivitiesByScreenTrackingId(
