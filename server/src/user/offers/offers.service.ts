...
export class OffersService {
  ...
  calcMonthlyPayment(
    ...
    if (apr) {
      ...
      const amortizationSchedule = this.mathExtService.makeAmortizationSchedule(
        ...
        requestId,
        moment().toDate(),
        this.getFirstPaymentDate(moment().toDate()),
      );
    ...
    monthlyPayment = parseFloat(parseFloat(String(monthlyPayment)).toFixed(7));
  ...
  async getPracticeOffers(
    loanAmount: number,
    screenTrackingDocument: ScreenTrackingDocument,
    requestId: string,
  ) {
    this.logger.log(
    ...
    const trades = (screenTrackingDocument.transUnion as TransUnionsDocument)
      .trade;
    if (!trades) {
      this.logger.error(
    ...
    const monthlyIncome = parseFloat((annualIncome / 12).toFixed(2));
    const monthlyDebt = this.transUnionService.getMonthlyTradeDebt(
      trades,
      requestId,
    );
    const adjRulesWeight = screenTrackingDocument.adjRulesWeight || 0;
    const practiceId = screenTrackingDocument.practiceManagement;
    const practiceManagement: PracticeManagementDocument = await this.practiceManagementModel.findOne(
      {
        _id: practiceId,
      },
    );
    ...
    let offers: LoanInterestRateDocument[] = await this.loanInterestRateModel
      .find({ stateCode })
      .sort({ term: 1 });
    if (!offers) {
      this.logger.error(
    ...
    let postDTI = result.preDTI.percent;
  ...
  async selectOffer(selectOfferDto: SelectOfferDto, requestId: string) {
    ...
    const screenTracking: ScreenTrackingDocument = await this.screenTrackingModel.findOne(
      {
