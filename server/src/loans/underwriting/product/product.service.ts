...
import { ProductRules, ProductRulesDocument } from './product-rules.schema';
import { AppService } from '../../../app.service';

@Injectable()
export class ProductService {
  constructor(
    @InjectModel(ProductRules.name)
    private readonly productRulesModel: Model<ProductRulesDocument>,
  ...
  async createPartnerRules(
    ...
    productId: string,
    requestId: string,
  ): Promise<ProductRulesDocument> {
    const partnerRules = this.configService.get<Record<string, unknown>>(
      'newPartnerRuleSet',
    );
    ...
    const productRules: ProductRulesDocument = await this.productRulesModel.create(
      partnerRules as any,
    );
    this.logger.log(
  ...
  async updatePartnerRules(
    currentRules: ProductRulesDocument,
    requestId: string,
  ): Promise<ProductRulesDocument> {
    this.logger.log(
    ...
    const availableRules = this.configService.get<Record<string, any>>(
      'newPartnerRuleSet',
    ).rules;
  ...
  async getStage1Rules(
    ...
    const ruleUserValueFuncs = {
      rule1: () => {
        ...
        let dob: string | Date = user.dateOfBirth;
        if (!dob) {
    ...
    const ruleUserValueFuncs = {
      ...
      rule5: (creditReport: any) => {
        const bankruptcyStartDate = moment()
          .startOf('day')
          .subtract(24, 'months');
        const bankruptcyTypes = this.configService.get<string[]>(
          'bankruptcyTypes',
        );
        let publicRecords =
          creditReport?.product?.subject?.subjectRecord?.custom?.credit
  ...
  async getProductRulesValue(
    ...
    let ruleDetails: ProductRulesDocument = await this.productRulesModel
      .findOne(ruleCriteria)
      .sort({ id: 1 });

    if (!ruleDetails) {
      await this.createPartnerRules(practiceId, productId, requestId);
      ruleDetails = await this.productRulesModel
        .findOne(ruleCriteria)
        .sort({ id: 1 });
    } else {
