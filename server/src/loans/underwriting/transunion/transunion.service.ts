...
export class TransunionService {
   ...
   * @param user User document
   */
  buildTransUnions(creditReport: any, user: UserDocument, requestId: string) {
    this.logger.log(
  ...
  async runCreditReport(
    ...
    let transUnionHistory: TransUnionHistoryDocument;
    ...
    const httpsAgent = new https.Agent({
      cert: fs.readFileSync(
        path.resolve(__dirname, `../${credentials.certificate.crtPath}`),
        'utf8',
      ),
      key: fs.readFileSync(
        path.resolve(__dirname, `../${credentials.certificate.keyPath}`),
        'utf8',
      ),
    ...
    try {
      ...
      try {
        ...
        transUnionHistory = await this.transUnionHistoryModel.create({
          user: user.id,
          requestData,
          status: 1,
    ...
    const transUnions = await this.transUnionsModel.create(
      transUnionsObj as any,
    );
  ...
  async getById(id: string): Promise<any> {
    const data = await this.transUnionsModel.findById(new ObjectID(id));
    return data;
  }
}
